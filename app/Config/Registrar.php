...
class Registrar
{
    ...
    public static $ModuleJson = APPPATH . "Modules.json";

    public static  function registerNamespaces() {
        $modules = file_get_contents(self::$ModuleJson);
        $modules = ( json_decode( $modules , true ) == NULL ) ? false : json_decode($modules) ;
        $psr4 = [];
        if (!($modules && is_array($modules) && count($modules))) {
            return $psr4;
        }
        foreach ($modules as $module) {
                $psr4[$module][] = self::$ModuleLocation . DIRECTORY_SEPARATOR . $module.DIRECTORY_SEPARATOR ;
        }
